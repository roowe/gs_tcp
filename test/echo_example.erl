...
-define(SOCKET_ALLOW_REC_ONE_MIN, 60).        % 心跳包超时次数
...
init([Socket]) ->
    ...
    case inet:setopts(Socket, 
                      [{delay_send, true},
                       {active, ?SOCKET_ALLOW_REC_ONE_MIN}]) of
        ok ->
            State = #reader{
                       socket = Socket                       
                      },
...
handle_info({tcp, Socket, Bin}, State) ->
    ?PRINT("~p~n", [Bin]),
    Ret = gen_tcp:send(Socket, Bin),
    ?PRINT("~p~n", [Ret]),
    {noreply, State};
handle_info({tcp_closed, _Socket}, StateData) ->
    ?PRINT("recv tcp_closed~n", []),
...
terminate(_Reason, State) ->
    (catch gen_tcp:close(State#reader.socket)),
    ok.
